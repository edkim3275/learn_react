...
- 변수명은 다른 사람이 볼때도 이해할 수 있도록 지정해주자 !

  현재 변수 `x`, `y`, `position.x`, `position.y` 처럼 유사해보이는 변수들이 존재하는데 기능에 따라서 명확히 이름을 정해주어야만 한다.

- 하나의 기능에 사용되는 변수들은 하나의 변수로 묶어서 만들어놓자.

  현재 sprite sheet 기법으로 캐릭터의 애니메이션을 설정해줄 때 필요한 변수들이 여러개존재한다.(`frameSet`, `fps`, `secondsToUpdate` `count`, `frameIndex`)

  이렇게 하나의 기능에 사용되는 변수들은 변수하나를 만들어서 묶어주도록 하자.

- 더 나아가서 다른 클래스에서도 사용될 수 있는 인스턴스라고 생각되면 새로운 클래스를 만들어서 사용하도록하자.

  sprite sheet는 player에서만 사용되는 것이아니라 map에서도 사용될 수 있다. 이 경우에 Sprite 클래스를 생성해서 Player, Map 클래스에서 상속받아서 사용할 수 있도록 만들어놓으면 재사용측면에 있어서 훨씬 용이해진다.
